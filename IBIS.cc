...
int FileOrGZ<gzFile>::printf(const char *format, ...)
{
	...
	if (buf_len + ret > buf_size - 1)
	{
		...
		{
			do
			{ // find the buffer size that fits the last vsnprintf() call
				buf_size += INIT_SIZE;
	...
	buf_len += ret;
	if (buf_len >= buf_size - 1024)
	{ // within a tolerance of MAX_BUF?
		// flush:
		gzwrite(fp, buf, buf_len);
		buf_len = 0;
	}
...
template <>
size_t FileOrGZ<gzFile>::pfwrite(const void *ptr, size_t size, size_t n)
{
	printf("GZip and Binary not currently supported together");
	exit(1);
	return 0;
}
...
void HomozygAndMiss::addBits(uint64_t h1, uint64_t h2, uint64_t m)
{ //Places single-bit values in the input into the bit sets.
	hom1 += h1;
	hom2 += h2;
	miss += m;
}

int HomozygAndMiss::getMissCount()
{ //Return count of missing sites for the current sample's window.
	return (int)__builtin_popcountl(miss);
}

HomozygAndMiss *seek(HomozygAndMiss *data, uint64_t indiv, uint64_t newMarkerBlocks)
{ //Finds the starting location for a given individual in the data. Later HomozygAndMiss classes for that individual can be reached by adding 1 to the pointer.
...
void SegmentData::updateSegmentEndpoints(int newEndPos, int newError, int newIBD1Error, int newMissingness)
{
	...
	{ //Won't update real endpoint if there was an error in the window, but tracks the information anyway for error analyses.
...
void SegmentData::selfErase()
{ //TODO This may be streamlinable. Will check later.
	startPos = -1;
	startFloat = -1;
	endPos = -100;
	endFloat = -1;
		...
		for (int m = 0; m < Marker::getNumChromMarkers(chrIndex); m++)
		{
			...
			for (int64_t b = 0; b < indBlocks64; b++)
			{
				...
				else
				{
					...
					{ //Handle if the second set of individuals does not fill out a set of 64 completely and needs to be supplemented with 0s.
	...
	for (uint64_t indiv2 = indiv1 + 1; indiv2 < ((uint64_t)numIndivs); indiv2++)
	{
		...
		for (int chr = 0; chr < Marker::getNumChroms(); chr++)
		{
			...
			for (uint64_t markerWindow = chromWindowBoundaries[chr][0]; markerWindow <= chromWindowBoundaries[chr][1]; markerWindow++)
			{
				...
				if (ibd2)
				{ //The check for if IBD2 is enabled
					if (isIBD2)
					{
						...
						if (windowErrorCount2 > 0)
						{
							...
							{
								if (currentIBD2Segment.errorCheck(windowErrorCount2, missingnessCount, currentEndPos, errorThreshold2))
								{ //Update potential endpoint if error threshold passed.
					...
					{ //If no IBD, attempt to store segment if valid.
						bool realSeg = endSegment(currentIBD2Segment, min_length2, min_markers2, storedSegs, totalIBD1Length, totalIBD2Length);
						if (realSeg)
						{
							handleIBD1PostIBD2(currentIBD1Segment, storedSegs, totalIBD1Length, totalIBD2Length, missingnessCount);
		...
		if (relCoef >= min_coef)
		{ //Only prints if current pair is sufficiently related.
			if (lock)
				omp_set_lock(lock);
			if (printCoef)
			{
				...
				{
					for (classVal = 1; classVal < 8; classVal++)
					{ //find degree of relatedness
